...
function loadAllBibles(): BibleVerse[] {
  ...
  const configs = [
    ...
    { file: 'weymouth.json',version: 'WEY' },
    { file: 'wycliffe.json',version: 'WYC' },
  ]
...
export async function initDatabase() {
  ...
  const targetBible = [
    ...
    !!findData('weymouth.json') && 'weymouth',
    !!findData('wycliffe.json') && 'wycliffe',
  ].filter(Boolean).join(',')
