...
const C = {
  ...
  glowP: 'var(--glow-p)', glowPS: 'var(--glow-p-strong)', glowG: 'var(--glow-g)', glowGS: 'var(--glow-g-strong)',
}
    ...
    <div style={{flex:1,padding:40,overflowY:'auto',background:C.tex1}}>
      <div style={{maxWidth:560}}>
        <div style={{display:'flex',alignItems:'center',gap:20,marginBottom:36}}>
          <svg width="64" height="64" viewBox="0 0 100 100" style={{filter:`drop-shadow(0 0 8px ${C.glowPS}) drop-shadow(0 0 16px ${C.glowG})`}}>
            <defs>
              <radialGradient id="ab1" cx="50%" cy="30%" r="70%"><stop offset="0%" stopColor="#241f42"/><stop offset="100%" stopColor="#0e0c1a"/></radialGradient>
            ...
            <circle cx="50" cy="50" r="40" fill="none" stroke={C.g2} strokeOpacity="0.3" strokeWidth="1"/>
...
export default function App() {
  ...
  const [activeTimer,setActiveTimer]     = useState<{label:string,targetTime:number}|null>(null)
  ...
  useEffect(()=>{
    ...
    async function load(){
      ...
      try{setStageMonitorOpen(!!await(window as any).shogunos.isStageMonitorOpen())}catch{}
    }
    load()
  },[showSplash])
  ...
  // ── STAGE MONITOR: push state out ───────────────────────────────────────
  // Same shape of data as the remote push above, reframed for a platform-
  // facing confidence monitor: current content, a section label, and
  // whatever's at the front of the queue as "up next." No-ops harmlessly in
  // the main process if the stage window isn't open.
  useEffect(()=>{
    if(showSplash)return
    const nextItem = queue[0]
    ;(window as any).shogunos.pushStageState({
      live, blankScreen,
      content: sections[currentSection]?.content || selectedVerse?.text || '',
      currentLabel: sections.length>1 ? `Section ${currentSection+1} of ${sections.length}` : 'Now Showing',
      nextTitle: nextItem?.title || null,
      nextType: nextItem?.type || null,
      timer: activeTimer,
    })
  },[showSplash,live,blankScreen,currentSection,sections,selectedVerse,queue,activeTimer])
  ...
  async function goLive(title:string,lyrics:string,ds?:Partial<DisplaySettings>){
    ...
    setLive(title);setBlankScreen(false);setActiveTimer(null)
  ...
  async function startCountdown(){
    ...
    setActiveTimer({label:timerLabel||'Countdown',targetTime})
  ...
  async function handleClear(){setLive(null);setBlankScreen(false);setActiveTimer(null);await(window as any).shogunos.closeLive()}

  async function toggleStageMonitor(){
    if(stageMonitorOpen){
      await(window as any).shogunos.closeStageMonitor()
      setStageMonitorOpen(false)
      notify('Stage monitor closed')
    }else{
      // Prefer a display other than whatever the congregation output is
      // using — the whole point is a second, private view for the platform.
      const stageDisplay = displays.find(d=>d.id!==selectedDisplay)?.id
      await(window as any).shogunos.openStageMonitor(stageDisplay)
      setStageMonitorOpen(true)
      notify(stageDisplay!=null?'Stage monitor opened on second display':'Stage monitor opened (drag it onto a monitor)')
    }
  }

  async function handleBlank(){
    const next=!blankScreen;setBlankScreen(next);if(next)setActiveTimer(null)
    ...
    <div style={{display:'flex',flexDirection:'column',height:'100vh',background:'transparent',fontFamily:"-apple-system,'Segoe UI',system-ui,'Inter',sans-serif",overflow:'hidden',color:C.t1,fontSize:13,position:'relative'}}>
      <style>{`
        *{box-sizing:border-box}
        ...
        .shimmer-btn{position:relative;overflow:hidden;transition:box-shadow 0.2s ease}
        ...
        .shimmer-btn:not(:disabled):hover{box-shadow:0 0 20px ${C.glowPS}, 0 0 8px ${C.glowG}}
        .shimmer-btn.glass-primary:not(:disabled):hover{box-shadow:inset 0 1px 0 rgba(255,255,255,0.4), 0 8px 20px rgba(10,6,26,0.4), 0 0 20px ${C.glowPS}, 0 0 10px ${C.glowG}}
      ...
      <div className="glass-bar" style={{height:68,borderBottom:`1px solid ${C.b0}`,display:'grid',gridTemplateColumns:'auto 1fr auto',alignItems:'center',flexShrink:0,zIndex:10,position:'relative'}}>
        ...
        <div style={{display:'flex',alignItems:'center',height:'100%'}}>
          <div style={{width:76,height:'100%',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,borderRight:`1px solid ${C.b0}`}}>
            <svg width="30" height="30" viewBox="0 0 100 100" style={{filter:`drop-shadow(0 0 5px ${C.glowPS}) drop-shadow(0 0 9px ${C.glowG})`}}>
              <defs>
                <linearGradient id="lg1" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={C.g3}/><stop offset="100%" stopColor={C.g1}/></linearGradient>
                <linearGradient id="lg2" x1="0" y1="0" x2="1" y2="1"><stop offset="0%" stopColor={C.p2}/><stop offset="50%" stopColor={C.g2}/><stop offset="100%" stopColor={C.p2}/></linearGradient>
              </defs>
              <circle cx="50" cy="50" r="46" fill="none" stroke="url(#lg2)" strokeWidth="2.5" style={{animation:'spin 14s linear infinite',transformOrigin:'50% 50%'}} strokeDasharray="8 6" strokeOpacity="0.55"/>
              <circle cx="50" cy="50" r="46" fill="none" stroke="url(#lg2)" strokeWidth="2"/>
        ...
        {/* Stage monitor */}
        <button className="glass-btn" onClick={toggleStageMonitor} title={stageMonitorOpen?'Close Stage Monitor':'Open Stage Monitor'}
          style={{color:stageMonitorOpen?C.p2:C.g2,fontSize:14,width:38,height:38,display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,marginRight:10,
            boxShadow:stageMonitorOpen?`0 0 0 1px ${C.p2} inset`:'none'}}>▤</button>
        {/* Dark mode toggle */}
          ...
          {NAV.filter(([gid])=>gid!=='service').map(([gid,gLabel])=>{
            ...
            return (
              <button key={gid} onClick={()=>setNavGroup(gid as NavGroup)} className={active?'':'nav-icon'}
                style={{display:'flex',alignItems:'center',gap:7,padding:'8px 15px',background:active?`linear-gradient(135deg,${C.g2},${C.g1})`:'none',border:'none',borderRadius:999,color:active?'#fff':C.t3,cursor:'pointer',fontSize:12.5,fontWeight:active?600:500,letterSpacing:'0.02em',whiteSpace:'nowrap' as const,transition:'all 0.15s',flexShrink:0,boxShadow:active?`inset 0 1px 0 rgba(255,255,255,0.3), 0 0 14px ${C.glowGS}`:'none'}}>
        ...
        <div style={{flex:1,display:'flex',flexDirection:'column',minWidth:0,background:C.tex0}}>
          <div style={{padding:'12px 20px',borderBottom:`1px solid ${C.b0}`,display:'flex',alignItems:'center',justifyContent:'space-between',flexShrink:0}}>
            ...
            <div style={{display:'flex',alignItems:'center',gap:6}}>
              {live&&<div className="live-dot" style={{width:6,height:6,borderRadius:'50%',background:C.live,boxShadow:`0 0 4px ${C.live}, 0 0 10px ${C.glowPS}, 0 0 18px ${C.glowP}`}}/>}
