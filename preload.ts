...
contextBridge.exposeInMainWorld('shogunos', {
  ...
  isStageMonitorOpen:   () => ipcRenderer.invoke('is-stage-monitor-open'),
  pushStageState:       (state: any) => ipcRenderer.send('stage-state-update', state),
  onUpdateStage:        (cb: (data: any) => void) => { ipcRenderer.removeAllListeners('update-stage'); ipcRenderer.on('update-stage', (_e: any, d: any) => cb(d)) },
  onStageClosed:        (cb: () => void) => { ipcRenderer.removeAllListeners('stage-closed'); ipcRenderer.on('stage-closed', () => cb()) },
  onStageDisplayChanged:(cb: (info: any) => void) => ipcRenderer.on('stage-display-changed', (_e: any, d: any) => cb(d)),
