...
function getStageHtmlPath(): string {
  const candidates = [
    path.join(__dirname, `../renderer/${MAIN_WINDOW_VITE_NAME}/stage.html`),
    path.join(__dirname, '../../src/stage.html'),
    path.join(process.resourcesPath || '', 'stage.html'),
  ]
  for (const p of candidates) {
    if (fs.existsSync(p)) return p
  }
...
// A second output window, separate from the congregation-facing liveWindow,
// meant for the platform/stage: current + next content, a clock, and the
// countdown timer if one's running. Operators typically feed this to a
// small monitor or TV facing the musicians/speaker rather than the projector.
//
// On a genuine multi-display rig this behaves like liveWindow — borderless,
// sized to fill whichever screen it's assigned. But unlike the live output,
// a stage monitor is just as useful on a single-display machine (e.g. testing,
// or a small church running the whole service off one laptop screen split
// with a second window) — so unlike liveWindow it never goes frameless/
// always-on-top there: it opens as an ordinary movable, resizable window the
// operator can drag onto a monitor themselves.
const createStageWindow = (displayId?: number) => {
  const displays = screen.getAllDisplays()
  const singleDisplay = displays.length <= 1
  const target = singleDisplay ? null : resolveTargetDisplay(displayId)

  stageWindow = new BrowserWindow({
    x: target?.bounds.x, y: target?.bounds.y,
    width: target?.bounds.width ?? 900, height: target?.bounds.height ?? 560,
    frame: singleDisplay,
  ...
  stageWindow.loadFile(getStageHtmlPath())
  if (target) stageWindow.setBounds(target.bounds)

  stageWindow.once('ready-to-show', () => {
    stageWindow?.show()
    mainWindow?.webContents.send('stage-display-changed', { displayId: target?.id ?? null })
  })
...
app.on('ready', async () => {
  ...
  // ── STAGE MONITOR ────────────────────────────────────────────────────────
  ipcMain.handle('open-stage-monitor', (_e, displayId?: number) => {
  ...
  ipcMain.handle('move-stage-to-display', (_e, displayId: number) => {
    if (!stageWindow) return
    const d = screen.getAllDisplays().find(x => x.id === displayId)
    if (!d) return
    stageWindow.setBounds(d.bounds)
  })
  ipcMain.handle('is-stage-monitor-open', () => !!stageWindow)
  // Fire-and-forget, like remote-state-update — the stage monitor just wants
  // whatever's current, and this can fire on every keystroke/slide change.
  ipcMain.on('stage-state-update', (_e, s: any) => { stageWindow?.webContents.send('update-stage', s) })
